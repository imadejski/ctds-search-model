...
def read_data(cosine_path):
    """
    Reads in csvs with cosine similarity scores, ground-truth labels for labeled test set
    """
    cosine_df = pd.read_csv(cosine_path)
    test_labels_df = pd.read_csv(
        "/opt/gpudata/mimic-cxr/mimic-cxr-2.1.0-test-set-labeled.csv"
    )
    cosine_df = cosine_df[cosine_df["study_id"].isin(test_labels_df["study_id"])]

    return cosine_df, test_labels_df
...
def count_positive_cases(labels, test_labels_df):
    """
    Finds total number of positive cases, represented by 1, in df w labels
    """
    n_counts = {}
    for label in labels:
        label_positives = test_labels_df[test_labels_df[label] == 1]
...
def calculate_accuracy_and_top_k(
    ...
    for label in labels:
        ...
        for emb in filtered_embedding_types:
            ...
            label_positives = test_labels_df[test_labels_df[label] == 1]["study_id"]
...
def resample_and_calculate_accuracies(
    ...
    for i in range(num_iterations):
        ...
        sampled_test_labels_df = test_labels_df[
            test_labels_df["study_id"].isin(sampled_ids)
        ]

        # Recalculate the number of positive cases in the sampled data
        n_counts_sampled = count_positive_cases(labels, sampled_test_labels_df)
...
def main():
    ...
    cosine_df, test_labels_df = read_data(args.cosine_path)
    cosine_df_transformed = transform_cosine_df(cosine_df, labels, embedding_types)
    n_counts = count_positive_cases(labels, test_labels_df)
    ...
    if args.resampling:
        ...
        save_results(
            mean_ci_results_df,
            args.output_results_path.replace(".csv", "_resampling_labeled.csv"),
        )

        # Save all resampling results in separate files
        save_results(
            all_results_df,
            args.output_results_path.replace(".csv", "_all_resampling_labeled.csv"),
        )
