...
def find_cosine_similarity(img_embedding, search_query_embedding):
    ...
    """
    img_embedding_reshaped = img_embedding.reshape(1, -1)
...
def main():
    ...
    labels = [
        "Blood Stagnation",
        "Bronchiectasis and Infection",
        "Bronchitis",
        "Bullae",
        "COVID-19 Ground Glass",
        "Calcification",
        "Chronic Bronchitis with Infection",
        "Clavicle Fracture (staleness)",
        ...
        "Honeycomb Lung Tuberculosis",
        "Hydropneumothorax",
        "Infection",
        "Interlobar Pleurisy",
        "Interstitial Pneumonia",
        "Intraperitoneal Free Air",
        "Localized Diaphragmatic Eventration",
        "Lung Cyst/Bronchial Cyst",
        "Mass",
        "Metastases",
        "Miliary Tuberculosis and Other Disseminated Tuberculosis",
        "No Finding",
        "Nodule",
        ...
        "Pneumonia",
        "Pneumothorax",
        "Postoperative Denaturation",
        "Pulmonary Atelectasis",
        "Pulmonary Hilars Enlarged and Thicken",
        "Pulmonary Mesenchyme Denaturation",
        "Rib Deformity",
        "Rib Fracture",
        "Rib Fracture (staleness)",
    ...
    mimic_embedding_library_path = (
        "/home/imadejski/ctds-search-model/data/midrc_embedding_library.csv"
    )
    ...
    for label in labels:
        for i in range(1, n_search_queries + 1):
            new_columns[f"{label} query_{i}"] = [np.nan] * len(mimic_embeddings_pd)
        new_columns[f"{label} avg_emb"] = [np.nan] * len(mimic_embeddings_pd)
        new_columns[f"{label} max_cossim"] = [np.nan] * len(mimic_embeddings_pd)

    new_columns_df = pd.DataFrame(new_columns)
    mimic_embeddings_pd = pd.concat([mimic_embeddings_pd, new_columns_df], axis=1)
    ...
    for label in tqdm(labels, desc="Processing labels"):
        ...
        for i in range(len(pos_search_queries)):
            mimic_embeddings_pd[f"{label} query_{i+1}"] = np.nan

        mimic_embeddings_pd[f"{label} avg_emb"] = np.nan
        mimic_embeddings_pd[f"{label} max_cossim"] = np.nan
