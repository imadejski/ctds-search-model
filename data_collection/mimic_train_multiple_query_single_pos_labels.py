...
def main():
    ...
    mimic_train_embedding_library_path = (
        "/home/imadejski/ctds-search-model/data/mimic_train_embedding_library.csv"
    )
    mimic_train_embeddings_pd = img_embeddings_df(mimic_train_embedding_library_path)
    ...
    for label in labels:
        ...
        for i in range(len(pos_search_queries)):
            mimic_train_embeddings_pd[f"{label} cosine_similarity_{i+1}"] = np.nan

        mimic_train_embeddings_pd[f"{label} average_embedding"] = np.nan
        mimic_train_embeddings_pd[f"{label} average_cosine_similarity"] = np.nan
        mimic_train_embeddings_pd[f"{label} max_cosine_similarity"] = np.nan

        for index, row in mimic_train_embeddings_pd.iterrows():
    ...
    output_file_path = "/home/imadejski/ctds-search-model/data/mimic_train_multiple_pos_cosine_similarity.csv"
    mimic_train_embeddings_pd.to_csv(output_file_path, index=False)
