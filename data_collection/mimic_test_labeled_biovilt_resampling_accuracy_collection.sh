...
for model in "${model_configs[@]}"; do
    ...
    if [ ! -f "$output_path/embedding_library.csv" ]; then
        echo "  Need embedding library."
    fi

    # Check if cosine_similarity_labeled.csv exists
    if [ ! -f "$output_path/cosine_similarity_labeled.csv" ]; then
        # Run general_mimic_cosine_similarity.py
        python data_collection/general_mimic_cosine_similarity_labeled.py "$model_checkpoint_path" "$output_path/embedding_library.csv" "$output_path/cosine_similarity_labeled.csv" "$split_type"
    else
        echo "  Skipping general_mimic_cosine_similarity_labeled.py: cosine_similarity_labeled.csv already exists."
    fi
    ...
    if [ ! -f "$output_path/model_top_n_accuracy_results_labeled.csv" ]; then
        ...
        python data_collection/general_mimic_accuracy_resampling_labeled.py \
            --cosine-path "$output_path/cosine_similarity_labeled.csv" \
